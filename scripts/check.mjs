...
for(const file of ['app.js','sync.js','sync-engine.js','sw.js','config.js'])execFileSync(process.execPath,['--check','www/'+file]);
const html=readFileSync('www/index.html','utf8'),app=readFileSync('www/app.js','utf8');
const version=app.match(/const APP_VERSION = "([^"]+)"/)[1];
for(const name of ['app.js','sync.js','sync-engine.js','styles.css'])assert.ok(html.includes(name+'?v='+version),name+' version mismatch');
